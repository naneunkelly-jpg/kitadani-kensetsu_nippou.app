...
import { updateMaterialAction, type FormState } from "../actions";
...
export function MaterialEditForm({
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-2xl border border-border bg-card p-6">
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-foreground">材料名</label>
        <input
          name="name"
          defaultValue={name}
          required
      ...
      <div>
        <label className="mb-1 block text-sm font-medium text-foreground">単位</label>
        <input
          name="unit"
          defaultValue={unit}
          className="w-full rounded-xl border border-border px-4 py-3 text-base outline-none focus:border-accent"
