...
export default async function ReportDetailPage({
  ...
  const { data: report } = await supabase
    .from("daily_reports")
    .select("id, employee_id, report_date, status, today_summary, tomorrow_plan, remarks")
    .eq("id", id)
    .single();

  if (!report || report.employee_id !== user.id) {
    notFound();
  }
  ...
  const entriesWithPhotos = await Promise.all(
    (entries ?? []).map(async (e) => {
      ...
      return { ...e, clientName: client?.name ?? "", worksiteName: worksite?.name ?? "", photoUrls: urls };
    })
  );
